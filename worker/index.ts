/**
 * Cloudflare Worker — serves the unified RTS Agents web build and proxies
 * API requests to their respective providers while handling CORS and
 * authentication. API keys are passed from the client in headers.
 *
 * Static assets (the Vite build in dist/renderer) are served by the Workers
 * Assets layer before this handler runs; non-asset requests fall through to
 * this worker, which proxies /api/* and defers everything else back to ASSETS
 * so the SPA fallback can render index.html.
 */
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Non-API routes: assets are normally handled before the worker runs, but
    // anything that falls through (SPA routes, missing files) defers to the
    // asset server so single-page-application fallback applies.
