...
| OpenCode         | `opencode --version`; launch with `opencode run --dir <repo>`         | CLI on PATH or configured command                          | Use non-interactive `run` (not bare `opencode` TUI). Pass `--dangerously-skip-permissions` for unattended edits. |
