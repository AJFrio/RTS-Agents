...
export default function ModelSelector({ value, onChange }: ModelSelectorProps) {
  ...
  const flatList = useMemo(() => {
    return Object.entries(groupedModels).flatMap(([, items]) => items);
  }, [groupedModels]);
    ...
    <div className="relative" ref={containerRef}>
      <div className="flex items-center bg-white dark:bg-black/40 border border-slate-200 dark:border-border-dark rounded-lg focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
        <input
          ...
          className="flex-1 bg-transparent border-none text-sm py-2.5 px-4 text-slate-800 dark:text-white focus:ring-0 placeholder:text-slate-500 w-full outline-none"
        />
      ...
      {isOpen && (
        <div
          className="absolute z-50 left-0 right-0 mt-1 max-h-60 overflow-y-auto bg-white dark:bg-[#1A1A1A] border border-slate-200 dark:border-border-dark rounded-lg shadow-xl"
          ref={listRef}
        >
