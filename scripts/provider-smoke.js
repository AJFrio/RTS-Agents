...
function classify(result) {
  ...
  const text = `${result?.status || ''} ${result?.error || ''} ${result?.message || ''}`.toLowerCase();
  ...
  if (text.includes('404') || text.includes('not found') || text.includes('unsupported')) return 'stale_contract';
...
async function main() {
  ...
  const probes = [
    ['jules', () => apiKeys.jules ? julesService.testConnection() : providerHealth.notConfigured('jules')],
    ['cursor', () => apiKeys.cursor ? cursorService.testConnection() : providerHealth.notConfigured('cursor')],
    ['codex', () => apiKeys.codex
      ...
      : codexInstalled
        ...
        : providerHealth.notConfigured('codex', { installed: false, message: 'OpenAI API key not configured and Codex CLI not found' })],
    ['claude-cloud', () => apiKeys.claude ? claudeService.testConnection() : providerHealth.notConfigured('claude-cloud')],
    ['github', () => apiKeys.github ? githubService.testConnection() : providerHealth.notConfigured('github')],
    ['jira', () => apiKeys.jira ? jiraService.testConnection() : providerHealth.notConfigured('jira')],
    ['openrouter', () => apiKeys.openrouter ? openrouterService.testConnection() : providerHealth.notConfigured('openrouter')],
    ['cloudflare', async () => {
