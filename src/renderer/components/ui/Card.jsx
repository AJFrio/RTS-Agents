...
export function AgentCard({ children, onClick, className = '' }) {
  return (
    <button
      type="button"
      className={`flex h-full w-full flex-col justify-start text-left agent-card rounded-xl shadow-sm hover:shadow-md active:scale-[0.98] p-4 transition-all duration-200 ${className}`}
      onClick={onClick}
    >
      {children}
    </button>
