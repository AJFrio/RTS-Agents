  ...
  ipcMain.handle('utils:get-status', async () => {
    const [
      ...
      configStore.hasApiKey('openrouter')
        ...
        : Promise.resolve({ success: false, error: 'Not configured' }),
      configStore.hasApiKey('openai')
        ? testOpenAiApiKeyConnection()
        : Promise.resolve({ success: false, error: 'Not configured' }),
      configStore.hasApiKey('gemini')
    ...
    const claudeCloudValid =
      claudeCloudStatus.status === 'fulfilled' && claudeCloudStatus.value.success;
    ...
    return {
      gemini: {
        ...
        error: geminiInstalled
          ...
          : geminiApiValid
            ? null
            : configStore.hasApiKey('gemini')
              ? geminiApiStatus.value?.error || 'Gemini API key invalid'
              : 'Gemini CLI not found & API Key missing',
      },
      openrouter: {
        ...
        error:
          openRouterStatus.status === 'fulfilled'
            ...
            : openRouterStatus.reason?.message,
      },
      openai: {
        ...
        error:
          openAiStatus.status === 'fulfilled'
            ? openAiStatus.value.error
            : openAiStatus.reason?.message,
      },
      jules:
      ...
      codex:
        codexStatus.status === 'fulfilled'
          ? codexStatus.value
          : { success: false, error: codexStatus.reason?.message },
      'claude-cli': {
