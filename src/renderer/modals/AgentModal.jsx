...
export default function AgentModal({ agent, onClose, api }) {
  ...
  return (
    <Modal open={!!agent} onClose={onClose}>
      <div
        id="agent-modal"
        className="bg-white dark:bg-sidebar-dark border border-slate-200 dark:border-border-dark w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl rounded-2xl"
      >
        <div className="p-6 border-b border-slate-200 dark:border-border-dark flex justify-between items-start bg-white dark:bg-black/40">
          <div className="flex-1 mr-8">
            ...
            <div className="mt-1 text-[10px] technical-font text-slate-500">
              Task overview and activity
            </div>
          </div>
          <button
            type="button"
        ...
        >
          ...
          {!loading &&
            ...
            (() => {
              const hasContent =
                typeof details.content === 'string' && details.content.trim().length > 0;
              if (hasContent) {
                return (
                  <div
          ...
          {!loading && !details && <p className="text-slate-500">No details available.</p>}
        </div>
