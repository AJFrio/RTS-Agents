...
| Cursor           | `GET https://api.cursor.com/v1/me`                                    | Basic auth with `apiKey:`                                  | v1 uses durable agents plus runs; list responses use `items`. Detail views call `GET /agents/{id}`, `GET /agents/{id}/runs`, and per-run `GET .../runs/{runId}` for `result` (prompts are not returned on read). |
...
| OpenCode         | `opencode --version`; `opencode run --dir <repo> --format json`; `opencode export <ses_id>`; TUI resume `opencode -s <ses_id>` | CLI on PATH or configured command | RTS stores internal tracking id plus OpenCode `ses_*` from JSONL stdout. Details load via `export`; terminal resume via `utils:open-opencode-session`. |
