...
/**
 * Placeholder shown while a follow-up turn is in flight, appended after the
 * feed items so the wait reads as the next turn in the same stream.
 */
function PendingRow({ label, assistantLabel }) {
  return (
    <div className="flex justify-start gap-2 py-1" role="status" aria-live="polite">
      <div
        aria-hidden="true"
      ...
      <div className="flex max-w-[78%] flex-col items-start gap-1">
        ...
        <div className="w-full rounded-2xl rounded-bl-sm bg-slate-100 px-3.5 py-2.5 dark:bg-slate-800">
 ...
 * section accordions. `pending` appends an in-flight turn placeholder while a
 * follow-up is being sent.
 */
export default function UnifiedActivityFeed({
  ...
  if (!feed?.length && !pending) return null;
  ...
  return (
    <div className="space-y-2">
      ...
      {pending && <PendingRow label={pending} assistantLabel={assistantLabel} />}
    </div>
  );
}
